...
const Index = () => {
  ...
  const rawBlogPosts = [
    { id: "ai-engineering-data-management-autodesk-vault-manufacturing", title: "AI in Engineering Data Management: How Autodesk Vault Is Changing Manufacturing Workflows", excerpt: "Discover how Autodesk Vault 2027.1 and AI-powered PDM are transforming engineering data management for manufacturing teams — faster access, reduced repetitive work and connected workflows.", image: "https://images.unsplash.com/photo-1581092921461-eab62e97a780?w=1600&q=80", date: "August 13, 2026", readTime: "11 min read", category: "Engineering Data" },
  ...
  const postT = blogPosts.find(p => p.id === "ai-engineering-data-management-autodesk-vault-manufacturing") || blogPosts[0];
  const postS = blogPosts.find(p => p.id === "gcc-it-infrastructure-ai-ready-operations-india") || blogPosts[1];
  const postR = blogPosts.find(p => p.id === "digital-twins-manufacturing-ai-real-time-3d-intelligent-factory") || blogPosts[2];
  const postQ = blogPosts.find(p => p.id === "it-asset-buyback-recover-value-protect-data") || blogPosts[3];
  const postP = blogPosts.find(p => p.id === "microsoft-security-copilot-ai-powered-enterprise-cybersecurity-2026") || blogPosts[4];
          ...
          <div className="flex gap-12">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10 mb-14 pb-14 border-b border-gray-100">
                <HeroCard post={postT} inView={featuredInView} />
                <div className="flex flex-col justify-between gap-0 pt-2 lg:pt-0">
                  {[postS, postR, postQ].map((post, i) => (
